...
class Repository:
    def __init__(
            self, name: str, namespace: str, ssh_url: str, http_url: str,
            localfs_url: pathlib.PosixPath = None, description: str = None):
        self.name = name
    ...
    def clone(self, base_path: pathlib.PosixPath, name: str = None):
        ...
        git.Git().clone(
            self.ssh_url, str(base_path / (name or self.name)), '--mirror')
    ...
    def clone_or_update(self, base_path: pathlib.PosixPath, name: str = None):
        ...
        if path.is_dir():
            ...
            try:
                ...
                pass
        LOG.info(
                'Repository directory does not exist or is not a '
                f'valid gir repo: {path}')
